...
f_max = f_0 * max_harm
plot_pad_time = 0
parameters = h5py.File("parameters.h5", "r")
time = parameters["/TDSE/t"][1:]
time_len = time.shape[0]
dt = time[1] - time[0]
"""
pulse_envelope = param_f["Pulse"]["Pulse_envelope_0"][1:]
pulse_time = param_f["Pulse"]["time"][1:]
...
for scale_type in ["lin"]:
    ...
    for num in w_list[rank::size]:
        ...
        y /= f_0
        if scale_type == "lin":
            plt.contourf(x, y, np.abs(scalogram), 15, cmap='viridis',vmax=0.014)
        elif scale_type == "log":
            min_val = 1e-6
